...
public class AcceptanceCertificateReturnGenerator {
    ...
    public void generateCertificate(String filePath, Map<String, String> data) throws Exception {
        ...
        addConditionInfo(document, data);
    ...
    private void addHeader(Document document, Map<String, String> data) throws DocumentException {
        Paragraph title = new Paragraph("Acceptance Certificate of Returned Rented Property", titleFont);
        title.setAlignment(Element.ALIGN_CENTER);
        document.add(title);
        document.add(Chunk.NEWLINE);

        Paragraph returnInfo = new Paragraph("City: " + data.get("city") + ", " + data.get("return_date"), regularFont);
        document.add(returnInfo);
        document.add(Chunk.NEWLINE);
    }

    private void addPartiesInfo(Document document, Map<String, String> data) throws DocumentException {
        String partiesInfo = "The Lessor, " + data.get("lessor_full_name") + " (hereinafter referred to the \"Lessor\"), " +
    ...
    private void addVehicleDetails(Document document, Map<String, String> data) throws DocumentException {
        String vehicleInfo = "The Lessee has returned, and the Lessor has accepted, the property subject to the rental agreement – a vehicle: " +
    ...
    private void addDocumentsInfo(Document document, Map<String, String> data) throws DocumentException {
        String documentsInfo = "The following documents have been returned with the vehicle:\n" +
    ...
    private void addConditionInfo(Document document, Map<String, String> data) throws DocumentException {
        String conditionInfo = "The technical condition of the vehicle at the time of return is " + data.get("condition_description") + ". " +
                "The Lessor has no claims regarding the condition of the property.";
        document.add(new Paragraph(conditionInfo, regularFont));
        document.add(Chunk.NEWLINE);
    }
